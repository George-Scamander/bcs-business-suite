...
export function PageTitleBar({ title, description, extra }: PageTitleBarProps) {
  ...
  return (
    <div
      className={`mb-5 flex rounded-xl border app-border app-surface px-4 py-3 sm:px-5 sm:py-4 ${
    ...
    >
      <div>
        ...
        {description ? <div className={`mb-0 app-text-soft ${isMobile ? 'text-xs' : 'text-sm sm:text-base'}`}>{description}</div> : null}
      </div>
      {extra ? (
