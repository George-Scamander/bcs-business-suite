...
-- Add Consignment as a valid payment method for sales orders.
-- Consignment carries a fixed 30-day consignment/credit period (payment_top_term = '30_DAYS').
-- The only function that needs updating is create_sales_order_with_auto_lead because it
-- contains the exhaustive payment-method allow-list. Direct table INSERT/UPDATE paths
-- already accept any text value for the payment_method column.
...
create or replace function public.create_sales_order_with_auto_lead(
  ...
  p_payment_top_term    text         default null
)
returns table (
  order_id      uuid,
  order_no      text,
...
declare
  ...
  v_payment_top_term    text        := nullif(trim(coalesce(p_payment_top_term, '')), '');
  v_lead_id             uuid;
  v_lead_code           text;
  v_order_id            uuid;
  v_order_no            text;
...
begin
  ...
  -- Validate payment method (CONSIGNMENT added as third valid option)
  if v_payment_method not in ('TOP', 'CASH', 'CONSIGNMENT') then
    raise exception 'Invalid payment method: %', v_payment_method;
  end if;
  if v_payment_method = 'TOP' and v_payment_top_term not in ('30_DAYS', '60_DAYS') then
    raise exception 'Payment top term required and must be 30_DAYS or 60_DAYS when payment method is TOP';
  end if;
  if v_payment_method = 'CASH' then
    v_payment_top_term := null;
  end if;
  -- Consignment always uses a fixed 30-day consignment period
  if v_payment_method = 'CONSIGNMENT' then
    v_payment_top_term := '30_DAYS';
  end if;
  ...
  begin
    perform public.record_operation_log(
      ...
      jsonb_build_object(
        ...
        'payment_top_term',    v_payment_top_term
      ),
      null
    );
  exception when others then
...
-- Re-grant execute so both old and new signatures remain accessible
grant execute on function public.create_sales_order_with_auto_lead(text, timestamptz, text, jsonb, uuid, text, text) to authenticated;
