...
-- Fix: update create_sales_order_with_auto_lead to support X_OWL category and
-- accept payment_method / payment_top_term parameters (both added after the last
-- version of this function was written).
...
begin
  ...
  if v_payment_method not in ('TOP', 'CASH') then
...
-- Re-grant execute so both old and new signatures are accessible
