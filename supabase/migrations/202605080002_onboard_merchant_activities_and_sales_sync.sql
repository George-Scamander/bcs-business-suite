...
do $$
begin
  create type public.merchant_activity_type as enum (
    'RENOVATION_TASK',
    'DELIVERY',
    'SALES_ORDER',
    'NOTE',
...
do $$
begin
  create type public.merchant_activity_status as enum (
    'PLANNED',
    'IN_PROGRESS',
    'DONE',
    'CANCELLED'
...
create table if not exists public.onboard_merchant_activities (
  id uuid primary key default gen_random_uuid(),
  merchant_id uuid not null references public.onboard_merchants (id) on delete cascade,
  activity_type public.merchant_activity_type not null default 'NOTE',
  status public.merchant_activity_status not null default 'PLANNED',
  title text not null,
  detail text,
...
create index if not exists idx_onboard_merchant_activities_merchant on public.onboard_merchant_activities (merchant_id);
create index if not exists idx_onboard_merchant_activities_activity_at on public.onboard_merchant_activities (activity_at desc);
create index if not exists idx_onboard_merchant_activities_type on public.onboard_merchant_activities (activity_type);
create index if not exists idx_onboard_merchant_activities_status on public.onboard_merchant_activities (status);

alter table public.onboard_merchant_activities enable row level security;

drop policy if exists onboard_merchant_activities_select_policy on public.onboard_merchant_activities;
create policy onboard_merchant_activities_select_policy
on public.onboard_merchant_activities
...
using (
  ...
  and (
    ...
    or exists (
      ...
      where om.id = onboard_merchant_activities.merchant_id
        and om.deleted_at is null
        and (om.bd_owner_id = auth.uid() or om.created_by = auth.uid())
    )
  )
);

drop policy if exists onboard_merchant_activities_insert_policy on public.onboard_merchant_activities;
create policy onboard_merchant_activities_insert_policy
on public.onboard_merchant_activities
...
drop policy if exists onboard_merchant_activities_update_policy on public.onboard_merchant_activities;
create policy onboard_merchant_activities_update_policy
on public.onboard_merchant_activities
...
drop policy if exists onboard_merchant_activities_delete_policy on public.onboard_merchant_activities;
create policy onboard_merchant_activities_delete_policy
on public.onboard_merchant_activities
...
grant select, insert, update, delete on public.onboard_merchant_activities to authenticated;

drop trigger if exists trg_onboard_merchant_activities_set_updated_at on public.onboard_merchant_activities;
create trigger trg_onboard_merchant_activities_set_updated_at before update on public.onboard_merchant_activities
